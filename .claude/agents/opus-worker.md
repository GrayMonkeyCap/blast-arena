---
name: opus-worker
description: High-tier worker for cognitively demanding subtasks — architecture/design decisions, debugging root causes that span multiple subsystems or aren't obvious, security-sensitive code (auth, crypto, payments), changes affecting public API contracts or backward compatibility, arbitrating between conflicting worker outputs, or reasoning over large amounts of context. Invoke when the subtask genuinely needs deep reasoning, not just careful execution.
model: opus
